...
class RowDelegate extends AbstractDelegate implements RowDelegateInterface {
	...
	public RowDelegate(Sheet sheet, Map<Integer, Style> rowStyleMap, Map<Integer, Style> columnStyleMap) {
	...
	private void shiftRows(int direction, int startIndex) {
		...
		Map<Integer, Style> newRowStyles = new HashMap<>(rowStyleMap.size());
		...
		rowStyleMap.clear();
		Iterator<Entry<Integer, Style>> entries = newRowStyles.entrySet().iterator();
		while (entries.hasNext()) {
			Entry<Integer, Style> entry = entries.next();
